...
public class PickerRecyclerView extends RecyclerView implements OnItemClickedListener, View.OnClickListener {
    ...
    public void init(Context context, final int daysToPlus, final int initialOffset, final int mBackgroundColor, final int mDateSelectedColor, final int mDateSelectedTextColor, final int mTodayDateTextColor, final int mTodayDateBackgroundColor, final int mDayOfWeekTextColor, final int mUnselectedDayTextColor) {
        ...
        post(new Runnable() {
            ...
            public void run() {
                ...
                pickerAdapter = new PickerAdapter((int) itemWidth, PickerRecyclerView.this, getContext(), daysToPlus, initialOffset, mBackgroundColor, mDateSelectedColor, mDateSelectedTextColor, mTodayDateTextColor,
                        mTodayDateBackgroundColor,
                        mDayOfWeekTextColor,
                        mUnselectedDayTextColor);
                setAdapter(pickerAdapter);
                LinearSnapHelper snapHelper = new LinearSnapHelper();
                snapHelper.attachToRecyclerView(PickerRecyclerView.this);
    ...
    private void selectItem(boolean isSelected, int position) {
        pickerAdapter.getItem(position).setSelected(isSelected);
        pickerAdapter.notifyItemChanged(position);
        if (isSelected) {
            listener.onDateSelected(pickerAdapter.getItem(position));
        }
    }
